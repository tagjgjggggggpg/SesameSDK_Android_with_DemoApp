...
class LockGroupActivity : AppCompatActivity() {
    ...
    private fun buildContentView(): ScrollView {
        val padding = dp(20)
        val content = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; gravity = Gravity.CENTER_HORIZONTAL; setPadding(padding, padding, padding, padding) }
        groupNameView = TextView(this).apply { textSize = 26f; gravity = Gravity.CENTER }
        stateView = TextView(this).apply { textSize = 18f; gravity = Gravity.CENTER; setPadding(0, dp(12), 0, dp(20)) }
        membersView = TextView(this).apply { textSize = 14f; setPadding(0, 0, 0, dp(20)) }
        lockButton = Button(this).apply { text = "すべて施錠"; setOnClickListener { runGroupAction(GroupLockAction.LOCK) } }
        unlockButton = Button(this).apply { text = "すべて解錠"; setOnClickListener { runGroupAction(GroupLockAction.UNLOCK) } }
        configureButton = Button(this).apply { text = "対象ロックを選択"; setOnClickListener { showDeviceSelection() } }
        resultView = TextView(this).apply { textSize = 14f; setPadding(0, dp(20), 0, 0) }
        val match = LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT)
        content.addView(groupNameView, match); content.addView(stateView, match); content.addView(membersView, match); content.addView(lockButton, match); content.addView(unlockButton, match); content.addView(configureButton, match)
        if (BuildConfig.DEBUG) {
            content.addView(Button(this).apply { text = "診断ログをコピー"; setOnClickListener { copyDiagnosticLog() } }, match)
            content.addView(Button(this).apply { text = "診断ログを消去"; setOnClickListener { P2DiagnosticLog.clear(); Toast.makeText(this@LockGroupActivity, "診断ログを消去しました", Toast.LENGTH_SHORT).show() } }, match)
        }
    ...
    private fun copyDiagnosticLog() {
        val text = P2DiagnosticLog.snapshot()
        if (text.isBlank()) { Toast.makeText(this, "診断ログはまだありません", Toast.LENGTH_SHORT).show(); return }
        val clipboard = getSystemService(Context.CLIPBOARD_SERVICE) as ClipboardManager
        clipboard.setPrimaryClip(ClipData.newPlainText("SESAME Phase 2 diagnostics", text))
        Toast.makeText(this, "診断ログをコピーしました (${text.lineSequence().count()}行)", Toast.LENGTH_SHORT).show()
    }
    ...
    private fun runGroupAction(action: GroupLockAction) {
        ...
        operationRunning = true; resultView.text = if (action == GroupLockAction.LOCK) "施錠中…" else "解錠中…"; updateButtons()
