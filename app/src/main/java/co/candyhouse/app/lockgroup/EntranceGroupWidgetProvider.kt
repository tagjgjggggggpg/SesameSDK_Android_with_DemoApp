...
class EntranceGroupWidgetProvider : AppWidgetProvider() {
    ...
    companion object {
        fun updateAll(context: Context, snapshot: EntranceGroupStateSnapshot, status: String?) {
            ...
            val edit = prefs.edit()
                .putString(KEY_A, snapshot.deviceA.state.name).putString(KEY_B, snapshot.deviceB.state.name)
                .putInt(KEY_BATTERY_A, snapshot.deviceA.batteryPercent ?: -1).putInt(KEY_BATTERY_B, snapshot.deviceB.batteryPercent ?: -1)
                .putString(KEY_CONFIRMATION, snapshot.confirmation.name)
            if (snapshot.confirmation != SnapshotConfirmation.NONE) edit.putLong(KEY_CONFIRMED_AT, snapshot.capturedAt)
            edit.apply()
            val manager = AppWidgetManager.getInstance(context)
            val component = ComponentName(context, EntranceGroupWidgetProvider::class.java)
            manager.getAppWidgetIds(component).forEach { updateOne(context, manager, it, cachedSnapshot(context), status) }
        }
        ...
        internal fun cachedSnapshot(context: Context): EntranceGroupStateSnapshot {
            ...
            val confirmation=runCatching{SnapshotConfirmation.valueOf(p.getString(KEY_CONFIRMATION,SnapshotConfirmation.NONE.name)?:SnapshotConfirmation.NONE.name)}.getOrDefault(SnapshotConfirmation.NONE)
            val fresh=confirmation!=SnapshotConfirmation.NONE
            return EntranceGroupStateSnapshot(EntranceDeviceSnapshot(state(KEY_A),battery(KEY_BATTERY_A),fresh),EntranceDeviceSnapshot(state(KEY_B),battery(KEY_BATTERY_B),fresh),p.getLong(KEY_CONFIRMED_AT,0L))
        }
        ...
        private fun updateOne(context:Context,manager:AppWidgetManager,id:Int,s:EntranceGroupStateSnapshot,status:String?){
            ...
            val confirm=when{time==null->"確認できません";s.confirmation==SnapshotConfirmation.PARTIAL->"一部確認 $time";else->"確認 $time"}
        ...
        private const val PREFS="entrance_group_widget";private const val KEY_A="last_device_a_state";private const val KEY_B="last_device_b_state";private const val KEY_BATTERY_A="last_battery_a";private const val KEY_BATTERY_B="last_battery_b";private const val KEY_CONFIRMED_AT="last_confirmed_at";private const val KEY_CONFIRMATION="last_confirmation"
    }
}
